...
class IRSystem():
    ...
    def _print_inverted_index(self):
        ...
        for word, tree in self._inverted_index.items():
            print('{}: {}'.format(
                word, [doc_id for doc_id in tree.tree_data() if doc_id != None]))
        print()
    ...
    def process_query(self, query):
        ...
        results_stack = []
        # get query in postfix notation as a queue
        postfix_queue = collections.deque(self._parse_query(query))
        ...
        while postfix_queue:
            ...
            if (token != 'AND' and token != 'OR' and token != 'NOT'):
                ...
                result = BooleanModel.and_operation(
                    left_operand, right_operand)   # evaluate AND

            elif (token == 'OR'):
                right_operand = results_stack.pop()
                left_operand = results_stack.pop()
                result = BooleanModel.or_operation(
                    left_operand, right_operand)    # evaluate OR

            elif (token == 'NOT'):
                right_operand = results_stack.pop()
                result = BooleanModel.not_operation(
                    right_operand, indexed_docIDs)  # evaluate NOT
